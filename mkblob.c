...
/*   for(i = 1; i < argc; ++i) 
 ...
 //  }
   ...
   /* Added on Raspberry Pi, seems writing of file is 
      extreme slow and we have tom make sure it's finish
      Tested on Raspi version 3. */
   system("sleep 5");  
  ...
  /* execfp = fopen ("mkblobmove.sh", "w");
   fprintf (execfp, "#!/bin/bash\n");
   fprintf (execfp, moveblob);
   fprintf (execfp, "\n");
   fprintf (execfp, rmdir);
   fprintf (execfp, "\n");
   fclose (execfp);   
   system("chmod 755 mkblobmove.sh");
   //system("./mkblobmove.sh"); */
